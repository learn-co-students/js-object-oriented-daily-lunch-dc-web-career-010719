...
class Neighborhood {
  ...
  customers() {
    const customers = this.deliveries().map(d => d.customer())
    return [...new Set(customers)]
  }

  meals() {
    const meals = this.deliveries().map(d => d.meal())
...
class Customer {
  ...
  totalSpent() {
    return this.deliveries().reduce((total, d) => {
      return total + d.meal().price
    }, 0)
  }
}
